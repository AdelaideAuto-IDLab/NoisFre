...
for idxn = 1:length(n_list)
    ...
    path=sprintf('%s/25C/TEST_%d.bin',folder,0);
    [ChhipHW_ref,bitmap_ref]=HWOnlyRead(path,n,'SEQ');
%     path=sprintf('%s/80C_15M/TEST_%d.bin',folder,0);
    path=sprintf('%s/25C/TEST_%d.bin',folder,4);
...
%     sreq_vector (idxd) = Nsel/Nsample/totalBit*1024*8;
    ...
    legend('Eqn.(2)','Random test','Chip test');
    s = sprintf('n = %d',n);
...
%     ytickformat('percentage');
    legend('Eqn.(6)','Random test','Chip test');
    s = sprintf('n = %d',n);
    title(s);
    xlabel('Theshold \theta');
    ylabel('Selection Rate (bit/Kbyte) SR_{HW}');
